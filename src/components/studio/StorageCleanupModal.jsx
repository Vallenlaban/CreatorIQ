  ...
  const handleUpdateRetention = async (hours) => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        setSuccessMessage(`✓ Pengaturan disimpan: File akan otomatis dibersihkan jika lebih dari ${hours} jam.`);
        setTimeout(() => setSuccessMessage(null), 4000);
      } else {
        setErrorMessage(data.message || 'Gagal menyimpan pengaturan.');
      }
    } catch (err) {
  ...
  const handleRunCleanup = async (forceAll = false) => {
    try {
      ...
      const res = await fetch('/api/cleanup/run', {
        ...
        body: JSON.stringify({
          forceAll,
          reason: forceAll ? 'Pembersihan manual total semua cache' : 'Pembersihan manual file kedaluwarsa'
        })
      });
      ...
      if (res.ok && data.success) {
        setSuccessMessage(
          `✓ Pembersihan berhasil! ${data.filesDeleted} file dihapus (${data.formattedFreed} ruang dibebaskan).`
        );
        fetchStatus();
        setTimeout(() => setSuccessMessage(null), 5000);
      } else {
        setErrorMessage(data.message || 'Gagal menjalankan pembersihan.');
      }
    } catch (err) {
      setErrorMessage('Terjadi kesalahan saat menjalankan pembersihan.');
    } finally {
      setActionLoading(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted mt-0.5">
                Pengelolaan otomatis file video MP4, klip 9:16 Shorts, dan thumbnail AI
              </p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 flex flex-col justify-between">
              ...
              <div className="text-[11px] text-text-muted mt-1">
                {storage.totalFiles} file total di server
              </div>
            </div>

            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 flex flex-col justify-between">
              <div className="flex items-center justify-between text-xs text-text-muted mb-2">
                <span>Jadwal Auto-Clean</span>
                <Clock className="w-3.5 h-3.5 text-accent-purple" />
              </div>
              <div className="text-2xl font-bold font-outfit text-white tracking-tight">
                {nextMinutes ? `~${nextMinutes} Menit` : 'Setiap 10m'}
              </div>
              <div className="text-[11px] text-text-muted mt-1">
                Pemeriksaan berkala terus berjalan
              </div>
            </div>

            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/10 flex flex-col justify-between">
              <div className="flex items-center justify-between text-xs text-text-muted mb-2">
                <span>Total Dibebaskan</span>
              ...
              <div className="text-[11px] text-text-muted mt-1">
                {cleanupStatus?.totalCleanedFiles || 0} file kedaluwarsa dibersihkan
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-3">
            <div className="flex items-center justify-between text-xs font-semibold text-text-secondary">
              <span>Rincian Folder Penyimpanan</span>
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02]">
                ...
                <div className="truncate">
                  <div className="text-text-muted text-[10px]">Shorts Video & Klip</div>
              ...
              <div className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02]">
                ...
                <div className="truncate">
                  <div className="text-text-muted text-[10px]">Thumbnail & Frame</div>
              ...
              <div className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02]">
                ...
                <div className="truncate">
                  <div className="text-text-muted text-[10px]">Upload Sementara</div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <label className="text-xs font-semibold text-white flex items-center gap-1.5">
                  ...
                  Durasi Penyimpanan Otomatis (Retention Time)
                </label>
                <p className="text-[11px] text-text-muted mt-0.5">
                  File yang sudah lebih lama dari durasi ini akan dibersihkan otomatis dari server secara permanen.
                </p>
              </div>
            </div>
          ...
          <div className="pt-2 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="text-xs text-text-muted flex items-center gap-1.5 w-full sm:w-auto">
              ...
              <span>Pembersihan otomatis berjalan setiap 10 menit di latar belakang.</span>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              ...
              >
                ...
                <span>Bersihkan File Kedaluwarsa</span>
              </button>
              ...
              <button
                ...
                disabled={actionLoading}
                onClick={() => {
                  if (window.confirm('Bersihkan semua file cache dan video sementara sekarang?')) {
                ...
                title="Hapus semua cache video dan thumbnail seketika"
              >
          ...
          {cleanupStatus?.recentLogs && cleanupStatus.recentLogs.length > 0 && (
            <div className="space-y-2 pt-2">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-text-muted flex items-center justify-between">
                <span>Riwayat Pembersihan Terbaru</span>
                <button
                  type="button"
                  onClick={fetchStatus}
                ...
                {cleanupStatus.recentLogs.map((log) => (
                  <div key={log.id} className="flex items-center justify-between text-text-muted hover:text-text-secondary py-0.5">
                    ...
                    <span className="text-emerald-400 font-medium">
                      {log.filesDeleted > 0 ? `+${log.formattedFreed} (${log.filesDeleted} file)` : '0 B (bersih)'}
                    </span>
                  </div>
                ))}
              </div>
