...
function execProcessAsync(
  cmd: string,
  args: string[],
  options?: { timeoutMs?: number; onStderr?: (chunk: string) => void; cwd?: string }
): Promise<{ code: number; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
    let stdout = "";
    let stderr = "";
    const proc = spawn(cmd, args, { cwd: options?.cwd, windowsHide: true });
...
export function formatAssTitle(raw: string): string {
  ...
  // If contains colon (e.g. "KELEMAHAN FATAL KAMERA ADVAN X1: BELUM ADA STABILISASI, DEAL BREAKER BUAT KONTEN TIKTOK!")
  if (clean.includes(":")) {
    ...
    if (header && body) {
      ...
      if (bodyWords.length > 7) {
        const mid = Math.ceil(bodyWords.length / 2);
        return `{\\c&H0000FFFF&}${header}:\\N{\\c&H00FFFFFF&}${bodyWords.slice(0, mid).join(" ")}\\N{\\c&H00FFFFFF&}${bodyWords.slice(mid).join(" ")}`;
      }
      return `{\\c&H0000FFFF&}${header}:\\N{\\c&H00FFFFFF&}${body}`;
    }
  }

  const words = clean.split(/\s+/).filter(Boolean);
  if (words.length <= 4) {
    return `{\\c&H0000FFFF&}${words[0]} {\\c&H00FFFFFF&}${words.slice(1).join(" ")}`;
  }

  // Split into 2 or 3 balanced lines depending on length
  if (words.length > 10) {
    const chunk1 = Math.ceil(words.length / 3);
    const chunk2 = Math.ceil((words.length - chunk1) / 2);
    const l1 = words.slice(0, chunk1).join(" ");
    const l2 = words.slice(chunk1, chunk1 + chunk2).join(" ");
    const l3 = words.slice(chunk1 + chunk2).join(" ");
  ...
  return `{\\c&H0000FFFF&}${line1}\\N{\\c&H00FFFFFF&}${line2}`;
}
...
export function generateModernAssSubtitleFile(
  ...
  // 1. Filter out any technical placeholder brackets or empty lines
  const validSubs = subtitles.filter((sub) => {
    const trimmed = (sub.text || "").trim();
    if (!trimmed) return false;
    if (trimmed.startsWith("[Audio Segmen") || trimmed.startsWith("[Speech") || trimmed.startsWith("[Audio")) return false;
    return true;
  });

  // 3. Dynamic sentence chunking: break long speech sentences into punchy 3-5 word subtitle beats
  const punchyItems: Array<{ start: number; end: number; text: string }> = [];
  for (const sub of validSubs) {
    const words = sub.text.trim().split(/\s+/).filter(Boolean);
    const subDuration = Math.max(0.6, sub.end - sub.start);

    if (words.length <= 5 || subDuration <= 2.2) {
      punchyItems.push(sub);
    } else {
      const chunkSize = 4;
      const totalChunks = Math.ceil(words.length / chunkSize);
      const timePerChunk = subDuration / totalChunks;
      for (let i = 0; i < totalChunks; i++) {
        const chunkWords = words.slice(i * chunkSize, (i + 1) * chunkSize);
        punchyItems.push({
          start: Math.round((sub.start + i * timePerChunk) * 100) / 100,
          end: Math.round((Math.min(sub.end, sub.start + (i + 1) * timePerChunk)) * 100) / 100,
  ...
  punchyItems.forEach((sub) => {
    ...
    const relativeEnd = Math.max(relativeStart + 0.5, sub.end - clipStartSec);

    const startStr = formatAssTime(relativeStart);
    const endStr = formatAssTime(relativeEnd);

    // Modern caption formatting: bold uppercase with alternating colored words (Yellow & White)
    ...
    words.forEach((w, wIdx) => {
      if (wIdx % 2 === 1) {
        ...
        styledText += `{\\c&H00FFFFFF&}${w} `; // White
      }
    });

    events += `Dialogue: 0,${startStr},${endStr},ModernCaption,,0,0,0,,${styledText.trim()}\n`;
  });
...
Style: ModernCaption,Liberation Sans,62,&H00FFFFFF,&H000000FF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,6.0,2.0,2,80,80,340,1
Style: TopHeaderTitle,Liberation Sans,44,&H00FFFFFF,&H000000FF,&H00000000,&H90000000,-1,0,0,0,100,100,1,0,1,6.0,2.5,8,70,70,220,1
Style: HookBanner,Liberation Sans,52,&H002EFFFF,&H000000FF,&H00000000,&H90000000,-1,0,0,0,100,100,0,0,1,6.0,2.0,8,60,60,220,1
 ...
 * Tries 4 tiers:
 * Tier 1: 9:16 layout with ASS subtitle filter.
 * Tier 2: 9:16 layout without ASS filter (if libass is unavailable).
  ...
  try {
    ...
    for (let i = 0; i < clipCount; i++) {
      ...
      validClips.push({
        ...
        thumbnailUrl: "",
        subtitles: clipSubs,
        subtitles_preview: clipSubs.map((s) => s.text).filter(Boolean).slice(0, 6)
      } as any);
    }
